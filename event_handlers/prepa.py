        ...
        for role in member.roles:
            if member is not None and eo in member.roles:
                ...
                if 'CIIC' in role.name:
                    log.debug(f"[CIIC] Role Found for {member.nick}")
                    cse_counselors.add(member)
                if 'INSO' in role.name:
                    log.debug(f"[INSO] Role Found for {member.nick}")
...
async def get_counselor_names(message: discord.Message):
    ...
    if split[0].upper() == '!EO' and split[1].upper() != "INEL" and split[1].upper() != "ICOM" and split[1].upper() != "INSO" and split[1].upper() != "CIIC":
        ...
        if split[0].upper() == "!EO":
            if split[1].upper() == "INEL":
                ...
                embed = discord.Embed(title="Estudiantes Orientadores de INSO",
                ...
                embed = discord.Embed(title="Estudiantes Orientadores de CIIC",
