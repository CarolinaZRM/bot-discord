...
def is_directed_to_bot(user_input):
    ...
    return re.match(r'^(bot).*', user_input) or re.search(r'(<@!719199208166522881>).*', user_input)
...
_TM_SONG_REGEX_ES = r'cu(a|á)l +es +(la +canci(o|ó)n|el himno) +(oficial )? *de +(tm|team made) *\?*'
_TM_SONG_REGEX_EN = r'what +is +the +(off?icial)? *(song|anthem) *of +(tm|team made) *\?*'
...
async def _cancion_team_made(message: discord.Message):
    ...
    if re.search(_TM_SONG_REGEX_ES, user_input) or re.search(_TM_SONG_REGEX_EN, user_input):
        await message.reply('La canción/himno de _Team MADE_ es **Grita por Jarabe de Palo**. '
