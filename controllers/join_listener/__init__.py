"""
//  /bot-discord/controllers/join_listener/__init__.py
//  py-bot-uprm
//
...
async def event_greet_new_member(client: discord.Client, member: discord.Member):
    log.info(
        "User joined |"
        f" ${member} ${member.display_name} ${member.name}${member.discriminator}"
    )

    # Greets you to server
    await member.send(
        f"*Bienvenido a UPRM y al Discord de TEAM MADE, {member.name}!* :tada:"
        " :raised_hands_tone3:\n**Por favor, dime cual es tu correo institucional para"
        " poder asignarte al grupo que Made eligió para ti!**\nEjemplo:"
    ...
    def check_same_user(client_response: discord.Message):
        ...
        log.info(
            f'[join.py] Client ResponseObj: "{client_response}" ||||| MemberObj:'
            f" {member}"
        )
    ...
    while student_info is None:
        await member.send(
            ...
            " email no aparece y estas seguro de que eres un prepa de INEL, ICOM, INSO o"
            " CIIC, comunícate con cualquier estudiante orientador del servidor de"
            ' discord. Tienen el rol de "EstudianteOrientador"'
        )
    ...
    student_full_name = (
        f"{student_info['first_name']} {student_info['middle_initial'] or ''} "
        f"{student_info['last_name']}"
    )
    ...
    log.debug(
        f"join_listener.greet {member.name}'s nickname was changed to"
        f" {student_full_name}"
    )
    ...
    await member.send(
        ...
        " secreto :eyes: ... Programar es super divertido y tu también puedes hacerlo!"
        " :hugging: "
    )
    ...
    message_to_send = (
        ...
        " estes aquí :tada:\nYo soy *MADE Bot* y sere tu *Bot* Consejero."
        " :smiley:\nEstoy aquí para ayudarte con cualquier duda que tengas.\nTe puedo"
        " ayudar a:\n\u2022 Encontrar edificios\n\u2022 Información de contacto para"
        " algunas oficinas importantes\n\u2022 Proveer 'links' muy útiles para tu"
        " carrera universitaria.\n\u2022 Y muchas cosas más!!!\n\nEspero ser de mucha"
        " ayuda :thumbsup:\n\nVamos a comenzar por escribir ***!help***.\nEste comando"
        " te mostrará la lista de algunas preguntas que me puedes hacer.\nEspero a que"
        " lo hagas..."
    )
    ...
    message_to_send = (
        f"Eso es {user_name}! :thumbsup: Ahí está la lista de algunos comandos"
        " rápidos.\nTodavía quedan más comandos con mucha información útil para ti"
        " :sweat_smile:Ahora intenta escribir ***!contactos***, este comando es mas"
        " específico y te proveéra una lista de todos los posibles contactos que tengo"
        " en mi banco de datos. Trátalo ahora :eyes:"
    )
    await member.send(content=message_to_send)
    ...
    closing = (
        f"{user_name}, eso es todo por hoy. Ya conoces los dos comandos mas importantes:"
        " ***'!help'*** y  ***'!contactos'***\nYa veras que estos te serán muy útiles,"
        " después puedes darme las gracias :sunglasses:\nAhora se te asigno un grupo en"
        " especifico de un personaje de Star Wars, lo puedes verificar en tu perfil\nEl"
        " grupo que te toco tiene un canal de texto y de voz para que puedas compartir"
        " con los otros miembros de tu grupo\nCualquier inconveniente le puedes"
        " escribir a Carolina Z. Rodriguez o Gabriel Santiago \n¡Hasta luego! También"
        " te digo que los Estudiantes Orientadores de Team MADE están para ayudarte, no"
        " dudes en ocuparlos para cualquier duda :grimacing:\n"
    )
...
async def assign_group(member: discord.Member, group_name: str, department_name: str):
    try:
        ...
        log.error(
            "Bot does not have permission to add roles. Could not add student"
            f' "{member}" to group.'
        )
        await member.send(
            "No pude asignarte a un grupo. Contacta a algún estudiante orientador e"
            " informarle de este error."
        )
